...
-- =============================================================================
-- USAGE: Run this entire file in Supabase SQL Editor (in order).
...
create table tenants (
  ...
  slug                      text unique not null,           -- e.g. "rentalworld"
...
create table branches (
  ...
  name                  text not null,                      -- e.g. "McAllen", "Laredo"
  code                  text,                               -- e.g. "MCL", "LRD" for invoice prefix
  address               text,
  city                  text,
  state                 text,
  zip                   text,
...
create table profiles (
  ...
  current_branch_id   uuid references branches(id),        -- where they're working NOW
  full_name           text,
...
create table equipment_categories (
  ...
  attributes              jsonb,                            -- [{key, label, type, options}]
  created_at              timestamptz default now()
);
...
create table equipment (
  ...
  inventory_item_id         text,                           -- legacy CPro reference
  name                      text not null,
  category                  text,
  ...
  dependencies              jsonb,                          -- [{equipmentId, name, reason, minQuantity}]
  image_url                 text,
...
create index idx_equipment_unit_status on equipment(unit_status);

-- =============================================================================
-- SECTION 4: CUSTOMERS
-- (Tenant-level — no branch_id. Branch context lives on rentals.)
-- =============================================================================
...
create table customers (
  ...
  linked_contacts             jsonb,                        -- [{name, role, phone, email, authorizedToRent}]
...
create table rentals (
  id                        uuid primary key default gen_random_uuid(),
  tenant_id                 uuid not null references tenants(id) on delete cascade,
  branch_id                 uuid not null references branches(id),   -- branch that wrote the contract
  ...
  equipment_name            text,
  source_branch_id          uuid references branches(id),            -- if cross-branch borrow
  ...
  status_history            jsonb,                          -- [{status, changedAt, changedBy, note}]
  notes                     text,
  created_at                timestamptz default now(),
  updated_at                timestamptz default now()
);
...
create table recurring_rentals (
  id                      uuid primary key default gen_random_uuid(),
  tenant_id               uuid not null references tenants(id) on delete cascade,
  ...
  customer_state          text,
  customer_zip            text,
  line_items              jsonb not null,                   -- [{equipmentId, equipmentName, quantity, dailyRate, ...}]
...
create table deliveries (
  ...
  driver_id                 text,
  driver_name               text,
  team_drivers              jsonb,                          -- [{driverId, driverName}]
  ...
  items                     jsonb,                          -- [{equipmentId, equipmentName, quantity, checked}]
  ...
  gps_coordinates           jsonb,                         -- {latitude, longitude, timestamp}
  photos                    jsonb,                          -- [{url, timestamp, gps, equipmentId, equipmentName}]
  signature_data_url        text,
...
create table recoveries (
  ...
  items                 jsonb,                             -- [{equipmentId, equipmentName, quantity}]
  scheduled_date        date not null,
  departed_at           timestamptz,
  arrived_at            timestamptz,
  completed_at          timestamptz,
...
create table work_orders (
  ...
  eta                     date,
  description             text,
  parts_required          jsonb,                           -- [{partName, quantity, cost, vendor, eta, received}]
  cost                    numeric(10,2),
...
-- =============================================================================
-- SECTION 8: FINANCIALS — Expenses, POs, Vendors, Cash Drawer
-- =============================================================================
...
create table purchase_orders (
  ...
  line_items              jsonb,                            -- [{supplyItemId, itemName, category, unit, qtyRequested, qtyReceived, unitPrice, lineTotal, notes}]
...
create table cash_drawers (
  ...
  attendant_log           jsonb,                           -- [{email, note, loggedBy, loggedAt}]
  closed_by               text,
...
-- =============================================================================
-- SECTION 9: DISCOUNTS, PROMOS, VOLUME RULES
-- =============================================================================
...
create table equipment_gps_links (
  id                      uuid primary key default gen_random_uuid(),
  tenant_id               uuid not null references tenants(id) on delete cascade,
...
-- =============================================================================
-- SECTION 14: PLATFORM-LEVEL (AIR SaaS — not tenant data)
-- =============================================================================

-- Waitlist entries (potential subscribers)
...
-- Subscriber trials (approved waitlist → active trial)
create table subscriber_trials (
  id                      uuid primary key default gen_random_uuid(),
  email                   text not null unique,
  company_name            text,
  contact_name            text,
  ...
  approved_by             text,
  approved_at             timestamptz,
  tenant_id               uuid references tenants(id),     -- set once they provision
  stripe_customer_id      text,
  stripe_subscription_id  text,
  notes                   text,
  created_at              timestamptz default now()
...
-- =============================================================================
-- RLS enforces tenant isolation. Branch filtering is done in app queries.
...
-- RLS Policy helper: all tenant-scoped tables use the same pattern.
-- tenant_id in JWT claim (set by custom_access_token hook) must match row's tenant_id.
...
-- Platform tables: service role only (no user-facing RLS needed)
alter table waitlist_entries enable row level security;
alter table subscriber_trials enable row level security;
alter table tenants enable row level security;
-- Only service role can access platform tables directly
...
begin
  ...
  if profile_row.id is null then
    return event;  -- User has no profile yet (first login), return unmodified
  end if;
...
-- =============================================================================
-- SECTION 17: HELPER FUNCTION — Auto-create profile on new user sign-up
-- This is triggered by Supabase Auth → Database Webhooks or via your
-- provisionTenant backend function after onboarding.
-- =============================================================================

create or replace function public.handle_new_user()
returns trigger language plpgsql security definer as $$
begin
  -- Profile is intentionally NOT auto-created here.
  -- Profiles are created by the provisionTenant backend function
  -- during onboarding, so tenant_id and branch_id can be properly assigned.
  return new;
end;
$$;
...
-- 1. In Supabase Dashboard → Auth → Hooks:
--    Add hook: "Customize Access Token" → function: public.custom_access_token
-- 2. Update your provisionTenant backend function to INSERT into:
--    tenants → branches → profiles (in that order)
-- 3. Replace base44.entities.X calls in your React app with supabase.from('x')
-- 4. Test with SupabaseTest page (/supabase-test)
-- =============================================================================