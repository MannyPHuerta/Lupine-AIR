...
export default function AuthCallback() {
  ...
  useEffect(() => {
    if (!supabase) { setStatus('Auth not available in preview mode.'); return; }
    ...
    if (code) {
      supabase.auth.exchangeCodeForSession(code).then(({ error }) => {
        if (error) { setStatus('Sign-in failed. The link may have expired. Please request a new one.'); return; }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-700 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-2xl p-10 flex flex-col items-center gap-4 max-w-sm w-full text-center">
        ...
        {status === 'Signing you in…' ? (
          <><Loader2 className="w-8 h-8 animate-spin text-blue-600" /><p className="text-slate-600 font-medium">{status}</p></>
        ) : (
