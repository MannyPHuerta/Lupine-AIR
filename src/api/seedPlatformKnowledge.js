/* global process */
// Vercel API route — seeds the platform_features table from the AIR training document
...
// The full AIR training document broken into seeded knowledge entries
...
The platform is organized into named Sections — each prefixed with "AI" — that correspond to major operational domains. Within each Section are Subsections (pages) that handle specific tasks.`,
    is_active: true,
    workflow: [],
    common_questions: [
...
Customer alerts: Credit hold shows a red banner requiring upfront payment. Blacklisted customers cannot be added to a rental.`,
  ...
  },
  {
    ...
    description: `Visual calendar showing which equipment is booked, available, or in conflict on any date range. Color coding: green = available, yellow = partially booked, red = fully booked/conflict. Click any block to see which rental occupies it. Buffer days appear as blocked time between rentals. If overbooking is enabled (per Availability Config), orange warning shown when thresholds are approached.`,
    is_active: true,
...
8. AI Nudge Panel suggests missing items ("You have 200 guests but no dance floor")
9. Save and optionally share with customer for review
10. Once approved, click Convert to Rental — all items pre-populate a rental contract
...
Mechanics see only their assigned work orders. They can update status, log labor hours, and request parts.
...
- Unusual discount patterns or excessive manager overrides
...
Geofence alerts: Set a radius (miles) per unit. If equipment leaves the worksite area, SMS and email alerts are sent to configured contacts (set in Company Settings → Fraud Alert Phones/Emails).`,
    is_active: true,
    ...
    common_questions: [
      ...
      { question: 'What triggers a fraud alert?', answer: 'Night movement, speed over 40 mph, geofence breach, customers with multiple disputes, and unusual discount or override patterns all trigger alerts.' },
    ],
  },
  {
    module: 'AIRoads',
...
6. Completes setup checklist (each item checked off)
...
Field Expense Capture: Drivers tap Log Expense, photograph receipt, AI pre-fills amount/vendor/category, driver confirms and submits.`,
    is_active: true,
...
6. Track bid status (submitted, awarded, lost) in RFQ Manager`,
    is_active: true,
...
Petty cash: Log cash taken out (fuel, supplies) as petty cash out with receipt photo. Log cash returned as petty cash in.`,
    is_active: true,
    ...
    common_questions: [
      { question: 'How do I open a cash drawer?', answer: 'Go to Cash Drawer → Open Drawer. Enter the starting float and the shift begins. All cash payments are tracked against this shift.' },
...
- Day 30: If still not upgraded, account suspended (lockout)
- Upgrade via subscription checkout → Stripe payment → account activates

Magic link sign-in: Users receive a one-click email link. No password required. Links expire in 1 hour.`,
    is_active: true,
    workflow: ['Submit waitlist', 'Admin approves', 'Click magic link', 'Complete onboarding (Company → Branch → Plan)', 'Trial starts', 'Upgrade before Day 14 to keep full access'],
    common_questions: [
      { question: 'How does the trial work?', answer: '14-day full access trial starts on approval. Day 12 you get a reminder. Day 14 if no upgrade, you drop to Core tier. Day 30, the account suspends. Upgrade anytime via the subscription checkout.' },
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (clearFirst) {
      await sb.from('platform_features').delete().eq('module', 'Platform');
      await sb.from('platform_features').delete().eq('module', 'AIRental');
      await sb.from('platform_features').delete().eq('module', 'AIREvents');
      await sb.from('platform_features').delete().eq('module', 'AIRepair');
      await sb.from('platform_features').delete().eq('module', 'AIRecovery');
      await sb.from('platform_features').delete().eq('module', 'AIRoads');
      await sb.from('platform_features').delete().eq('module', 'AIRfq');
      await sb.from('platform_features').delete().eq('module', 'Administration');
    }

    const rows = TRAINING_ENTRIES.map(e => ({
      module: e.module,
    ...
    return res.json({ success: true, inserted: data.length, entries: data.map(d => d.feature_name) });
  } catch (err) {
    console.error('[seedPlatformKnowledge] error:', err);
