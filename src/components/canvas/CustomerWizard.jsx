...
import { ArrowRight, ArrowLeft, Sparkles, Loader2, CheckCircle2, Upload } from 'lucide-react';
    ...
    <div className="flex items-center justify-center gap-2 mb-8">
      {STEPS.map((step, i) => (
        <div key={step.id} className="flex items-center gap-2">
          ...
          <span className={`text-xs font-medium ${i === current ? 'text-white' : 'text-white/30'}`}>{step.label}</span>
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-4">
        <div>
          ...
          <input className="w-full bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/30 outline-none focus:ring-2 focus:ring-cyan-500"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select value={data.eventType || 'other'} onChange={e => onChange({ eventType: e.target.value })}
          ...
          <div>
            ...
            <input type="date" value={data.eventDate || ''} onChange={e => onChange({ eventDate: e.target.value })}
              className="w-full bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none focus:ring-2 focus:ring-cyan-500" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="number" placeholder="150" value={data.guestCount || ''}
          ...
          <div>
            ...
            <input type="time" value={data.eventTime || ''} onChange={e => onChange({ eventTime: e.target.value })}
              className="w-full bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none focus:ring-2 focus:ring-cyan-500" />
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="relative">
              <input type="number" placeholder="100" value={data.venueWidthFt || ''}
            ...
            <div className="relative">
              <input type="number" placeholder="150" value={data.venueLengthFt || ''}
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button key={s.v} onClick={() => onChange({ venueSurface: s.v })}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[{ v: false, label: '☀️ Outdoor' }, { v: true, label: '🏠 Indoor' }].map(opt => (
              <button key={String(opt.v)} onClick={() => onChange({ isIndoor: opt.v })}
...
// Step 3: AI generates layout
function StepAI({ data, equipment, onGenerated, onBack }) {
  ...
  const generate = async () => {
    ...
    const catalogSummary = equipment
      .filter(e => e.unitStatus === 'available')
      .slice(0, 60)
      .map(e => ({ id: e.id, name: e.name, category: e.category, dailyRate: e.dailyRate,
    ...
    const prompt = `You are an expert event layout planner for a rental equipment company in the Rio Grande Valley, TX.
...
- Space: ${data.venueWidthFt || '?'} ft wide × ${data.venueLengthFt || '?'} ft long
- Venue: ${data.venueName || 'not specified'}

AVAILABLE EQUIPMENT (id, name, category, dailyRate, footprintW ft, footprintL ft):
${JSON.stringify(catalogSummary)}

Create a practical equipment layout for this event. Return ONLY a JSON array of items to place on the canvas. Each item must include:
- equipmentId (from the catalog above)
- quantity (integer)
- x (canvas x position in pixels, starting at 20)
- y (canvas y position in pixels, starting at 20)
- notes (brief note on placement)

Rules:
- Space items realistically (use footprintW/L × 10px/ft for sizing)
- Always include seating (chairs) appropriate for guest count
- Include tables (1 per 8 guests)
- If outdoor, recommend a generator if there are powered items
- Fit within ${(data.venueWidthFt || 80) * 10} × ${(data.venueLengthFt || 100) * 10} px canvas
- Return 4–10 items maximum
- Do not include items that don't exist in the catalog`;
    ...
    // Map AI response to canvas items
    ...
    setResult({ canvasItems, summary: response.summary });
    setLoading(false);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={generate} disabled={loading}
            ...
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Building your layout…</> : <><Sparkles className="w-4 h-4" /> Generate My Layout</>}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        {result.canvasItems.map((item, i) => (
          <div key={i} className="flex items-center gap-3 bg-slate-800 rounded-xl px-4 py-3">
            <div className="w-3 h-3 rounded-sm flex-shrink-0" style={{ backgroundColor: item.color }} />
            <div className="flex-1">
              <div className="text-sm text-white font-medium">{item.equipmentName}</div>
              {item.notes && <div className="text-xs text-white/40">{item.notes}</div>}
            </div>
            {item.quantity > 1 && <span className="text-xs text-white/50">×{item.quantity}</span>}
            {item.dailyRate > 0 && <span className="text-cyan-400 text-xs font-bold">${(item.dailyRate * item.quantity).toFixed(0)}/day</span>}
          </div>
        ))}
      </div>

      <div className="flex gap-3">
        <button onClick={() => setResult(null)} className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-white text-sm transition">
          <Sparkles className="w-4 h-4" /> Regenerate
        </button>
        <button onClick={() => onGenerated(result.canvasItems)}
          className="flex-1 flex items-center justify-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-black font-bold rounded-xl py-3 transition">
          View on Canvas <ArrowRight className="w-4 h-4" />
        </button>
      </div>
    </div>
  );
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto">
        {canvasItems.map((item, i) => (
          <div key={i} className="flex items-center gap-3 bg-slate-800 rounded-xl px-4 py-3">
            <div className="w-3 h-3 rounded-sm flex-shrink-0" style={{ backgroundColor: item.color }} />
            <div className="flex-1 text-sm text-white">{item.equipmentName}</div>
            {item.quantity > 1 && <span className="text-xs text-white/50">×{item.quantity}</span>}
            {item.dailyRate > 0 && <span className="text-cyan-400 text-xs font-bold">${(item.dailyRate * item.quantity).toFixed(0)}/day</span>}
          </div>
        ))}
      ...
      <div className="flex gap-3">
        ...
        <button onClick={onSubmit} disabled={submitting}
          ...
          {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Submitting…</> : <>Submit for Review <CheckCircle2 className="w-4 h-4" /></>}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-8 relative">
        <button onClick={onCancel} className="absolute top-4 right-4 text-white/30 hover:text-white text-lg">✕</button>
        ...
        {step === 2 && <StepAI data={data} equipment={equipment} onGenerated={handleGenerated} onBack={() => setStep(1)} />}
        {step === 3 && <StepReview canvasItems={canvasItems} onSubmit={handleSubmit} onBack={() => setStep(2)} submitting={submitting} />}
      </div>
    </div>
  );
}