...
import { Search, ChevronDown, ChevronRight, Loader2, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="h-full flex flex-col bg-slate-900 border-r border-white/10 w-64 flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {Object.keys(grouped).length === 0 && !isSearching && aiSuggestions.length === 0 && (
          <div className="p-4 text-xs text-white/30 text-center">No available items</div>
        )}
        {Object.keys(grouped).length === 0 && isSearching && (
          <div className="flex items-center justify-center gap-2 p-4 text-xs text-cyan-400">
            <Loader2 className="w-3.5 h-3.5 animate-spin" /> Searching alternate names…
          </div>
        )}
        {Object.keys(grouped).length === 0 && !isSearching && aiSuggestions.length > 0 && (
          <div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-cyan-400 border-b border-white/10">
              <Sparkles className="w-3 h-3" /> Did you mean…
            </div>
            {aiSuggestions.map(eq => (
              <div
                key={eq.id}
                draggable
                onDragStart={(e) => onDragStart(e, eq)}
                className="mx-2 my-1 px-3 py-2 rounded-lg border border-cyan-500/30 bg-slate-800 hover:bg-slate-700 cursor-grab active:cursor-grabbing transition"
              >
                <div className="text-xs text-white font-medium truncate">{eq.name}</div>
                <div className="text-cyan-400 text-[10px] mt-0.5">${eq.dailyRate}/day</div>
              </div>
            ))}
          </div>
        )}
