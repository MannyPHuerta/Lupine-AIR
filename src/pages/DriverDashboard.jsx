  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 mb-6">
        <div className="flex items-center gap-3 flex-wrap bg-slate-100 p-3 rounded-xl gap-y-2">
...
function DeliveryCard({ delivery, onSelect, onMarkReceived, markingReceived, currentDriver }) {
  ...
  return (
    <div className={`border rounded-xl transition-all shadow-sm ${scheduleChanged ? 'border-red-400 animate-flash' : statusClass}`}>
      <button onClick={onSelect} className="w-full text-left p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="font-bold text-lg leading-tight">{delivery.customerName}</div>
            <div className="text-sm opacity-75 mt-1.5 flex items-center gap-1.5">
              <MapPin className="w-4 h-4 flex-shrink-0" />
              {delivery.customerCity}, {delivery.customerState}
            </div>
            <div className="text-sm opacity-75 mt-1 flex items-center gap-1.5">
            ...
            {isTeamDelivery && (
              <div className="text-sm opacity-75 mt-1 flex items-center gap-1.5">
                ...
                {delivery.teamDrivers.map(t => t.driverName).join(', ')}
              </div>
            )}
            {delivery.scheduleChangedAt && (
              <div className="text-sm mt-2 flex items-center gap-1 font-bold text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-1.5">
                ⚠️ RESCHEDULED — was {delivery.previousScheduledDate || '?'}{delivery.previousScheduledTime ? ` ${delivery.previousScheduledTime}` : ''}
                <span className="font-normal text-red-500 ml-1">→ {delivery.scheduledDate}{delivery.scheduledTime ? ` ${delivery.scheduledTime}` : ''}</span>
              </div>
            )}
            {delivery.assignedAt && (
            ...
            <div className="text-sm opacity-75 mt-1">{delivery.items?.length || 0} item(s)</div>
          </div>
      ...
      {delivery.status === 'scheduled' && onMarkReceived && (
        <div className="px-4 pb-4 border-t border-current border-opacity-20 pt-3">
          {alreadyReceived ? (
            <div className="text-sm text-green-700 flex items-center gap-1.5 font-medium">
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full py-3 text-sm font-semibold rounded-lg bg-white bg-opacity-70 border border-current border-opacity-30 hover:bg-opacity-90 flex items-center justify-center gap-2 transition active:scale-95"
            >
...
function RecoveryCard({ recovery, onSelect }) {
  return (
    <button
      onClick={onSelect}
      className="w-full border rounded-xl p-4 text-left transition-all active:scale-[0.99] shadow-sm bg-rose-50 border-rose-200 text-rose-900"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-sm opacity-75 mt-1.5">{recovery.items?.length || 0} item(s) to recover</div>
        </div>
        <span className="text-sm font-semibold bg-white bg-opacity-70 px-3 py-1.5 rounded-full shadow-sm flex-shrink-0">
...
function StatusBadge({ status }) {
  const labels = {
    ...
    departed: '🚗 Departed',
    arrived: '📍 Arrived',
    setup_complete: '⚙️ Setup Done',
    signed: '✍️ Signed',
    completed: '✅ Done',
  };
  
  return <span className="text-sm font-semibold bg-white bg-opacity-70 px-3 py-1.5 rounded-full shadow-sm">{labels[status] || status}</span>;
}
